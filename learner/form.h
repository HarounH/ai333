...
public:
	...
	Move() { mmx_d = -1; my_shortest_path = -1;op_shortest_path=-1; eval=-1;}
	~Move() {}
	Move(int M, int R, int C) 	{ m=M; to.r = R; to.c = C; r=R; c=C; mmx_d = -1; my_shortest_path = -1; op_shortest_path=-1; eval=-1;}
...
// 		cout << "\n" ;
	...
	/* State evaluation. */
	double evaluate(vector<double> coeffs);
	pair< vector<double> , double > evaluate_values(vector<double> coeffs);
...
public:
	...
	double max_value(double a, double b, int cutoff, int curdept, float tl, vector<double> coeffs);
	double min_value(double a, double b, int cutoff, int curdept, float tl, vector<double> coeffs);
