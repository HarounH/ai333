...
Move Player::ordinary_mmx( int depth , float time_limit)
{
	...
	vector<double> current_coeffs = read_coeff_from_file() ;
	double backed_up = max_value( -INFTY, +INFTY , depth, 0, time_limit, current_coeffs);
	
	pair< vector<double> , double > eval_cur = locState.evaluate_values(current_coeffs);
	
	if (!((fabs(backed_up)>fabs(3.0*eval_cur.second)) or (fabs(backed_up) > 2000)))
	{
		adjust_coeff(eval_cur.first , current_coeffs , backed_up, eval_cur.second);
		write_coeff_to_file(current_coeffs);  
	}
	
	return best_move;
}

double Player::max_value(double alpha, double beta, int cutoff, int curDepth, float time_limit, vector<double> coeffs)
{
	//if (cutoff == curDepth or locState.i_won() or locState.i_lost() ) return locState.evaluate();
	if (cutoff == curDepth or locState.is_endgame() or (locState.i_won() /*and !locState.i_lost()*/)) return locState.evaluate(coeffs);
	...
	for (vector<Move>::iterator it = moves.begin() ; it!=mend ;it++)
	{
		...
		double cur_min = min_value(alpha,beta,cutoff,curDepth+1,time_limit,coeffs);
...
double Player::min_value(double alpha, double beta, int cutoff, int curDepth, float time_limit, vector<double> coeffs)
{
	//if (cutoff == curDepth or locState.i_won() or locState.i_lost()) return locState.evaluate();
	if (cutoff == curDepth or locState.is_endgame() or (locState.i_won() /*and !locState.i_lost()*/)) return locState.evaluate(coeffs);
	...
	for (vector<Move>::iterator it = moves.begin() ; it!=mend ;it++)
	{
		...
		v = min(v,max_value(alpha,beta,cutoff,curDepth+1,time_limit,coeffs));					// FIX time_limit
		if (v <= alpha) { 
			locState.unapply_move(t); //H - Changed some stuff to be VERY careful.
			return v;
		}
