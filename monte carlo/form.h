...
public:
	...
	void init( int _N , int _M , int _K , int _pn , float max_time );
	void read_move_from_client_cpp(int& m, int& r , int& c, MonteCarlo& mc);
	void send_move_to_client_cpp(int& m, int& r , int& c, MonteCarlo& mc);
	void read_time_left_from_client_cpp(float tl);
	
	void init_monte_carlo(MonteCarlo& mc);
